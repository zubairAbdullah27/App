...
export default function CreateProject({navigation}) {
    ...
    //user projects name go here
    const [projectName, setProjectName] = useState();


    //nextHandler
    const nextHandler=()=>{
        if(projectName == undefined || projectName.length<1){
            alert('Please Enter Proper Project Name')
        }else{
            navigation.navigate('uxTestName',{projectName:projectName})
        }
    }
...
const styles = StyleSheet.create({
    ...
    body: {
        flex: 6, backgroundColor: '#340E56', justifyContent: 'space-around', alignItems: 'center'
    },
    headerText: {
        fontSize: hp('2.3'), fontFamily: 'Montserrat-Regular', fontWeight: '800', marginTop: hp('4'), color: '#B8A4C7', marginRight: wp('10')
    },
    input: {
        fontFamily: 'Montserrat-Regular', fontWeight: '800', fontSize: hp('2'),color:'#D2EEC7',width:wp('40'),
    },
    footer:{
